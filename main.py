...
class MainApp(QMainWindow):
    ...
    def open_and_process_image(self):
        ...
        if file_path:  # Kiểm tra nếu người dùng chọn tệp
            ...
            height, width, channel = rgb_image.shape # Lấy chiều cao, chiều rộng và số kênh màu (RGB: 3 kênh)
            bytes_per_line = channel * width # Tổng số byte của một dòng ảnh
            qt_image = QImage(rgb_image.data, width, height, bytes_per_line, QImage.Format.Format_RGB888) # Tạo một đối tượng QImage từ dữ liệu pixel (rgb_image.data)
            ...
            self.lbl_anh.setPixmap(pixmap.scaled(
                label_width,
                label_height,
                QtCore.Qt.AspectRatioMode.KeepAspectRatioByExpanding, # Duy trì tỉ lệ ảnh gốc nhưng mở rộng theo kích thước QLabel
                QtCore.Qt.TransformationMode.SmoothTransformation # Làm mịn khi thay đổi kích thước ảnh
            ))
    ...
    def process_image(self, img):
        try:
            ...
            # Cắt vùng biển số xe
            (x, y, w, h) = cv2.boundingRect(number_plate_shape) # Xác định hình chữ nhật bao quanh đường viền
